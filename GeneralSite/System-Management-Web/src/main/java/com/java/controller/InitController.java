...
public class InitController {
	...
	if (!("").equals(accountName) && !("").equals(passWord)) {
	    ...
	    String name = p.readProperty("accountName");
	    String pass = p.readProperty("password");
	    String rtmsg = "";
	    // 帐号密码正确，开始初始化
	    if (accountName.equals(name) && passWord.equals(pass)) {
		// 1.清除原有数据
		try {
		...
		try {
		    ...
		    user.setIshiden(true);
		    user.setLastLoginIp(null);
		    user.setPhotograph("/user/uploadImg/aaa.jpg");
		    user.setUserName("admin");
		    List<Resource> list = this.resourceService.find(new Query());
		    user.setResource(list);
		    Role r = this.roleService.findOneById(roleId);
		    user.setRole(r);
