					...
					<div class="panel-body">
						<div class="row">
							...
							<div class="col-md-12">
								...
								<shiro:hasPermission name="user:author">
									<button class="btn btn-white btn-xs" onclick="return author()">
										...
										<tbody>
											...
											<c:forEach items="${pageList.datas}" var="items"
												...
												<tr>
													...
													<td>${items.lastLoginTime }</td>

													<td>${items.lastLoginIp }</td>
													...
													<td class="table-action-hide"><shiro:hasPermission
														...
														</shiro:hasPermission> <shiro:hasPermission name="user:author">
															<a href="javascript:void(0)"
																onclick="return author('${items.id}')"
																class="delete-row"><i title="权限管理"
								...
								<div class="col-md-6">
									<ul class="pagination pagination-split pull-right"
										...
										<li
											...
											href="${pageContext.request.contextPath}/user/list?pageNo=${pageList.upPage}&selectid=${selectid}&selectparentid=${selectparentid}">上一页</a></li>
										...
										<c:forEach items="${pageList.navigatepageNums}" var="nav">
											<c:choose>
												<c:when test="${nav == pageList.pageNo}">
													<!-- 1当前页面 -->
													<li class="active"><a
														href="${pageContext.request.contextPath}/user/list?selectid=${selectid}&selectparentid=${selectparentid}">${nav}</a>
													</li>
												</c:when>
												<c:otherwise>
													<li><a
														href="${pageContext.request.contextPath}/user/list?pageNo=${nav}&selectid=${selectid}&selectparentid=${selectparentid}">${nav}</a></li>
												</c:otherwise>
											</c:choose>

										</c:forEach>
										<li><a
											href="${pageContext.request.contextPath}/user/list?pageNo=${pageList.nextPage}&selectid=${selectid}&selectparentid=${selectparentid}">下一页</a></li>
									</ul>
								</div>
							</div>
